...
- The pointer must be returned by `heap_alloc`.
...
5. Track allocation in `alloced_chunks`.
...
- Only exact pointers returned by `heap_alloc()` may be freed.
